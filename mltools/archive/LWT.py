...
def make_wavelets(
    ...
    kx, ky = torch.meshgrid(k_arr, k_arr,indexing="ij")
...
def get_apodizer(N, dim, apstart=3 / 4):
    ...
    r = (
        torch.sqrt(
            (
                (torch.stack(torch.meshgrid(*[arr for _ in range(dim)],indexing="ij"), dim=0) - cent)
                ** 2
            ).sum(0)
        )
        / cent
...
def make_healpix_wavelets(N, NR=8, nside=2, dtype=torch.float64, rough_mem_limit=4000):
    ...
    k_arr = torch.fft.fftfreq(N, 1 / N).to(dtype=dtype)
    kx, ky, kz = torch.meshgrid(k_arr, k_arr, k_arr,indexing="ij")
